...
router.get('/', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const getDurartionInMinutesAndSeconds = (
      ticks: number
    ): [number, number] => {
    ...
    const formattedResults = results.map((item) => ({
      ...
      duration: item.RunTimeTicks
        ? getDurartionInMinutesAndSeconds(item.RunTimeTicks)
        : 0, // Convert ticks to seconds
      type: item.Type,
      image: item.ImageTags?.Primary
        ? `${JELLYFIN_URL}/Items/${item.Id}/Images/Primary?tag=${item.ImageTags.Primary}&maxWidth=200`
        : null,
    }));
